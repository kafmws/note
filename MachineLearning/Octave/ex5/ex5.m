...
% Map Xtest and normalize (using mu and sigma)
X_test = polyFeatures(Xtest, p);
X_test = bsxfun(@minus, X_test, mu);
X_test = bsxfun(@rdivide, X_test, sigma);
X_test = [ones(size(X_test, 1), 1), X_test];

J_test = 1/(2*size(Xtest, 1)) * sum((X_test * theta - ytest) .^ 2);
fprintf(['Cost of test set at lambda = 3: %f '...
